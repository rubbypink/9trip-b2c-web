...
export default function useHotelBooking({ pricingTable, hotel, checkIn, checkOut, nights }) {
  ...
  const getLineTotal = useCallback(
    ...
    [pricingTable, quantities, nights]
  );
   ...
   * @param {number} maxQty
   */
  const setRoomQuantity = useCallback(
    (roomId, rateType, delta, maxQty) => {
      setQuantities((prev) => {
        const key = `${roomId}_${rateType}`;
  ...
  }, [quantities, pricingTable, hotel, checkIn, checkOut, nights, cartItems, addItem, updateCartItem, removeCartItemByKey]);
