...
export default function HotelCard({ hotel, item }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-lg transition-shadow group">
      <Link href={`/hotels/${data.slug}`} target="_blank" rel="noopener noreferrer" className="block relative aspect-[4/3] overflow-hidden" data-service-type="hotel" data-service-id={data.id}>
        <Image
      ...
      <div className="p-4">
        ...
        <Link href={`/hotels/${data.slug}`} target="_blank" rel="noopener noreferrer" className="font-semibold text-gray-900 hover:text-blue-600 line-clamp-2 mb-1 min-h-[2.5rem]">
        ...
        <div className="flex items-center justify-between">
          <PriceDisplay
            ...
            label="Từ "
          />
          <Link href={`/hotels/${data.slug}`} target="_blank" rel="noopener noreferrer" className="px-3 py-1.5 bg-blue-600 text-white text-xs font-medium rounded-lg hover:bg-blue-700 transition-colors">
            Chi tiết
          </Link>
        </div>
      </div>
