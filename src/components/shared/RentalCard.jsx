...
export default function RentalCard({ rental, item }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-lg transition-shadow group">
      <Link href={`/rentals/${data.slug}`} className="block relative aspect-[4/3] overflow-hidden">
        <img
      ...
      <div className="p-4">
        <Link href={`/rentals/${data.slug}`} className="font-semibold text-gray-900 hover:text-blue-600 line-clamp-2 mb-1">
          {data.name}
        </Link>
