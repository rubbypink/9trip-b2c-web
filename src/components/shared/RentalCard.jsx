...
export default function RentalCard({ rental, item }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-lg transition-shadow group">
      <Link href={`/rentals/${data.slug}`} target="_blank" rel="noopener noreferrer" className="block relative aspect-[4/3] overflow-hidden" data-service-type="rental" data-service-id={data.id}>
        <Image
      ...
      <div className="p-4">
        <Link href={`/rentals/${data.slug}`} target="_blank" rel="noopener noreferrer" className="font-semibold text-gray-900 hover:text-blue-600 line-clamp-2 mb-1 min-h-[2.5rem]">
          {data.name}
        </Link>
        ...
        <div className="flex items-center justify-between border-t border-gray-50 pt-3">
          ...
          <Link href={`/rentals/${data.slug}`} target="_blank" rel="noopener noreferrer" className="text-xs font-medium text-blue-600 hover:underline">
            Xem thêm
          </Link>
        </div>
      </div>
