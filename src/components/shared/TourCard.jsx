...
export default function TourCard({ tour, item, variant = "grid" }) {
  ...
  if (variant === "list") {
    return (
      <div className="flex gap-4 bg-white rounded-xl border border-gray-100 p-4 hover:shadow-md transition-shadow">
        <Link href={`/tours/${data.slug}`} className="shrink-0 w-60 h-44 rounded-lg overflow-hidden">
          <img
        ...
        <div className="flex flex-col flex-1 min-w-0">
          <Link href={`/tours/${data.slug}`} className="text-lg font-semibold text-gray-900 hover:text-blue-600 line-clamp-2">
            {data.title}
          </Link>
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-lg transition-shadow group">
      <Link href={`/tours/${data.slug}`} className="block relative aspect-[4/3] overflow-hidden">
        <img
          src={data.featuredImage || "/placeholder-tour.jpg"}
          alt={data.title}
      ...
      <div className="p-4">
        <Link href={`/tours/${data.slug}`} className="font-semibold text-gray-900 hover:text-blue-600 line-clamp-2 mb-1">
          {data.title}
        </Link>
