...
export default function ReviewsPanel({ ratingAverage = 0, ratingCount = 0, reviews = [] }) {
  ...
  if (totalReviews === 0) {
    return (
      <div className="text-center py-10">
        ...
        <p className="text-gray-500 text-sm">Chưa có đánh giá nào cho tour này.</p>
      </div>
    );
  }
  ...
  return (
    <div>
      {/* Rating Summary */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 p-6 rounded-xl border border-gray-200 bg-gray-50/50">
        {/* Average score */}
        <div className="text-center">
          <div className="text-4xl font-bold text-gray-900">{ratingAverage.toFixed(1)}</div>
          ...
          {visibleReviews.map((review) => (
            <div key={review.id} className="rounded-xl border border-gray-200 p-5">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center text-sm font-medium text-gray-500 overflow-hidden">
                  {review.userAvatar ? (
