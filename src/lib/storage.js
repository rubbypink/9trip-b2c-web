...
export async function uploadGalleryImages(serviceType, serviceId, files) {
	return Promise.all(files.map((file, idx) => uploadGalleryImage(serviceType, serviceId, idx, file)));
}
...
export async function uploadRoomGalleryImages(serviceType, serviceId, roomId, files) {
	return Promise.all(files.map((file, idx) => uploadRoomGalleryImage(serviceType, serviceId, roomId, idx, file)));
}
...
export async function resolveDocImages(doc) {
	...
	if (result.rooms) {
		if (Array.isArray(result.rooms)) {
			...
			result.rooms = await Promise.all(result.rooms.map((room) => resolveDocImages(room)));
