...
export async function GET() {
				...
				try {
					...
					snap.docs.forEach((doc) => {
						...
						if (slug && !slugs.has(slug)) {
							slugs.add(slug);
							dynamicUrls.push({ path: `/${colName}/${slug}`, priority: '0.8', changefreq: 'weekly', lastmod: data.updatedAt?._seconds ? new Date(data.updatedAt._seconds * 1000).toISOString() : new Date().toISOString() });
						}
					});
				} catch {
	...
	const allUrls = [
		...
		...dynamicUrls.map((r) => ({ loc: `${baseUrl}${r.path}`, priority: r.priority, changefreq: r.changefreq, lastmod: r.lastmod })),
	];
...
${allUrls
	...
	return new Response(sitemap, { headers: { 'Content-Type': 'application/xml; charset=utf-8', 'Cache-Control': 'public, max-age=3600, s-maxage=3600' } });
}
