...
export default function RegisterPage() {
  ...
  async function handleRegister(e) {
    ...
    if (password !== confirmPassword) {
      setError({ code: "auth/password-mismatch", message: "Mật khẩu xác nhận không khớp." });
      return;
    }

    if (password.length < 6) {
      setError({ code: "auth/weak-password", message: "Mật khẩu phải có ít nhất 6 ký tự." });
      return;
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
          ...
          {error && <FirebaseErrorHandler error={error} onRetry={() => setError(null)} />}
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-sm"><span className="px-3 bg-white text-slate-400">hoặc</span></div>
          </div>

          <div className="space-y-3">
            <button type="button" onClick={() => handleSocialLogin("google")} disabled={!!socialLoading} className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-slate-300 rounded-xl hover:bg-slate-50 transition text-sm font-medium text-slate-700">
            ...
            <button type="button" onClick={() => handleSocialLogin("facebook")} disabled={!!socialLoading} className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-slate-300 rounded-xl hover:bg-slate-50 transition text-sm font-medium text-slate-700">
          ...
          <p className="text-center text-sm text-slate-500 mt-6">
            Đã có tài khoản?{" "}
            <Link href={`/login?redirect=${encodeURIComponent(redirect)}`} className="text-orange-600 hover:text-orange-700 font-semibold">Đăng nhập</Link>
          </p>
        </div>
      </div>
    </div>
