...
export const HOTEL_SCHEMA = {
	...
	pricing: { type: 'object', description: 'Basic pricing info', properties: { basePrice: { type: 'number', description: 'Base price in VND' }, currency: { type: 'string', description: 'Currency code (default: VND)' } } },
	...
	// Ratings
	rating: { type: 'object', description: 'Guest rating info', properties: { average: { type: 'number', description: 'Average guest rating 1-10' }, count: { type: 'number', description: 'Number of reviews' } } },
	...
	// Map
	map: { type: 'object', description: 'Map coordinates', properties: { lat: { type: 'number', description: 'Latitude' }, lng: { type: 'number', description: 'Longitude' }, zoom: { type: 'number', description: 'Zoom level' } } },
	...
	rooms: { type: 'array', description: 'Embedded array of room objects', items: { ref: 'HOTEL_ROOM_SCHEMA' } },
...
export const HOTEL_EXTRACT_SCHEMA = {
	...
	properties: {
		...
		rooms: { type: 'array', items: HOTEL_ROOM_SCHEMA, description: 'Array of room objects' },
		// Override reviews to use HOTEL_REVIEW_SCHEMA
		reviews: { type: 'array', items: HOTEL_REVIEW_SCHEMA, description: 'Array of review objects' },
	},
	required: ['name'],
};
...
export function MAP_TO_FIRESTORE(rawData) {
	...
	const amenityLower = (rawData.amenities || []).map((a) => (typeof a === 'string' ? a.toLowerCase() : ''));
	...
	const doc = {
		...
		// Pricing (basic only)
		pricing: { basePrice: rawData.pricing?.basePrice || rawData.pricing?.adultPrice || null, currency: rawData.pricing?.currency || 'VND' },
		...
		// Ratings
		rating: { average: rawData.rating?.average || rawData.ratingAverage || 0, count: rawData.rating?.count || rawData.ratingCount || 0 },
