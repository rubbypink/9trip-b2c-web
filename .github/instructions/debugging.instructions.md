...
4. **Kiểm tra schema liên quan**: Trong `packages/shared/schemas/`, tìm file schema tương ứng với collection/data đang xử lý (ví dụ `tours.js` cho Tour, `hotels.js` cho Hotel).
5. **Tham khảo `/AGENTS.md`**: Đọc lại các Absolute Constraints ở file `/AGENTS.md` để đảm bảo không vi phạm quy tắc hệ thống (JS only, App Router, JSDoc, Tailwind v4, etc.).
...
> **Ngoại lệ** Nếu bạn đã đọc hoặc được cung cấp trong ngữ cảnh sẵn hoặc nếu lỗi rõ ràng không liên quan kiến trúc hệ thống, schema, hoặc quy ước code ví dụ như lỗi chính tả, ngữ pháp, lỗi UI hoàn toàn, bạn có thể bỏ qua bước này. Tuy nhiên, nếu lỗi liên quan đến data access, UI rendering, hoặc logic liên quan đến kiến trúc, việc đọc Memory Bank là bắt buộc để đảm bảo fix đúng hướng.
...
6. **Kiểm tra Schema Compliance (RẤT QUAN TRỌNG):**
   ...
   - Luôn đối chiếu field access pattern với canonical schema trong `packages/shared/schemas/`:
     - Tours: `packages/shared/schemas/tours.js`
     - Hotels: `packages/shared/schemas/hotels.js`
     - Activities: `packages/shared/schemas/activities.js`
     - Bookings: `packages/shared/schemas/bookings.js`
     - Users: `packages/shared/schemas/users.js`
   - Đặc biệt kiểm tra:
...
- **Tuân thủ hệ thống rules:** Xem lại `/AGENTS.md` để đảm bảo fix không vi phạm Absolute Constraints (đặc biệt là JS only, App Router, không admin/partner, Tailwind v4, Firebase modular imports).
- **Ưu tiên đúng Schema:** Khi sửa lỗi liên quan đến dữ liệu, luôn dùng tên field và kiểu dữ liệu từ canonical schema trong `packages/shared/schemas/`. Nếu legacy data có field cũ, cần migration song song thay vì code workaround.
