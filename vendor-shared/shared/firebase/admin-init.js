...
const ADMIN_APP_NAME = '9trip-admin';
 ...
 * @returns {{ adminDb: import('firebase-admin/firestore').Firestore, adminAuth: import('firebase-admin/auth').Auth, adminStorage: import('firebase-admin/storage').Storage }}
 */
export function initFirebaseAdmin({ useLazyProxy = false } = {}) {
	function ensureAdminInit() {
		if (getApps().some(app => app.name === ADMIN_APP_NAME)) return true;
		try {
			...
			if (serviceAccount) {
				...
				initializeApp(config, ADMIN_APP_NAME);
				return true;
			}
			...
			if (useLazyProxy) {
				...
				initializeApp({ storageBucket }, ADMIN_APP_NAME);
			} else {
			...
			{},
			{
				get(_, prop) {
					if (!db) {
						...
						db = getFirestore(getApp(ADMIN_APP_NAME));
					}
					const value = db[prop];
			...
			{},
			{
				get(_, prop) {
					if (!auth) {
						...
						auth = getAuth(getApp(ADMIN_APP_NAME));
					}
			...
			{},
			{
				get(_, prop) {
					if (!storage) {
						...
						storage = getStorage(getApp(ADMIN_APP_NAME));
					}
	...
	ensureAdminInit();
	return { adminDb: getFirestore(getApp(ADMIN_APP_NAME)), adminAuth: getAuth(getApp(ADMIN_APP_NAME)), adminStorage: getStorage(getApp(ADMIN_APP_NAME)) };
}
...
export function getAdminDb() {
	...
	return getFirestore(getApp(ADMIN_APP_NAME));
}
...
export function getAdminStorage() {
	...
	return getStorage(getApp(ADMIN_APP_NAME));
}
