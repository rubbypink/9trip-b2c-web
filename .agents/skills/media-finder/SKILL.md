---
name: media-finder
description: Tìm kiếm, phát hiện, tạo ảnh/video chất lượng cao cho mọi nền tảng (Facebook, TikTok, Website, Blog, v.v.). Dùng web search, system storage, Google Drive, AI generation & editing (Gemini Interactions API, gemini-3.1-flash-image-preview / gemini-3-pro-image-preview). Tự động mở rộng phạm vi tìm kiếm, chấm điểm chất lượng, đề xuất tối ưu. Luôn dùng skill này khi người dùng cần tìm ảnh, video, tạo ảnh, tìm media cho social media, website, blog, hoặc bất kỳ nhu cầu media nào.
applyTo: "**"
---
...
3. **AI Generation** — Tạo và chỉnh sửa ảnh bằng Gemini Interactions API (gemini-3.1-flash-image-preview, gemini-3-pro-image-preview)
...
#### 2c. AI Generation & Editing (Gemini Interactions API)
Khi không tìm đủ kết quả hoặc cần chỉnh sửa ảnh, sử dụng Gemini Interactions API với `@google/genai`:
- Model: `gemini-3.1-flash-image-preview` (ưu tiên tốc độ, số lượng lớn) hoặc `gemini-3-pro-image-preview` (chất lượng chuyên nghiệp).
- Tạo ảnh mới (Text-to-Image): Truyền text prompt.
- Chỉnh sửa ảnh (Image-to-Image): Truyền ảnh base64 và text prompt mô tả thay đổi.
...
// Tạo ảnh mới
const interactionGen = await ai.interactions.create({
  model: "gemini-3.1-flash-image-preview",
  input: `High-quality ${media_type} of ${topic} in ${style_hint || 'professional'} style. Shot composition: ${platform === 'tiktok' ? 'vertical 9:16' : 'horizontal 16:9'}.`
});

// Chỉnh sửa ảnh (Text-and-image-to-image)
const interactionEdit = await ai.interactions.create({
  model: "gemini-3.1-flash-image-preview",
  input: [
    { type: "text", text: "Change the background to a sunset beach scene" },
